...
##SBATCH -x node[110]
#SBATCH --job-name=ankh_large_10_rounds
#SBATCH -n 1 
#SBATCH -N 1   
##SBATCH --gres=gpu:1
...
#SBATCH --mem=20gb  
#SBATCH --output /orcd/archive/abugoot/001/Projects/Matteo/Github/directed_evolution/grid_search/out/ankh_large_10_rounds-%j.out 
...
for name in "${names[@]}"; do
    datasets+=("${name}_ankh_large")
done

experiment_name="ankh_large"
num_simulations=10
num_iterations=10
...
function run_grid_search() {
    ...
    echo "Running ${dataset_name} dataset:" > out/${dataset_name}-${embedding_type_pt}-ankh_large_10_rounds.out
    python3 -u grid_search.py \
        --dataset_name ${dataset_name} \
        --experiment_name ${experiment_name} \
        --base_path ../extract/ankh/embeddings \
        ...
        >> out/${dataset_name}-${embedding_type_pt}-ankh_large_10_rounds.out
    echo "Done running ${dataset_name} dataset:" >> out/${dataset_name}-${embedding_type_pt}-ankh_large_10_rounds.out
}
