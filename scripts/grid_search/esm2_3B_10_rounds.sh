...
##SBATCH -x node[110]
#SBATCH --job-name=esm2_3B_10_rounds
#SBATCH -n 1 
#SBATCH -N 1   
##SBATCH --gres=gpu:1
...
#SBATCH --mem=20gb  
#SBATCH --output /om/group/abugoot/Projects/Matteo/Github/directed_evolution/grid_search/out/esm2_3B_10_rounds-%j.out 
...
for name in "${names[@]}"; do
    datasets+=("${name}_esm2_t36_3B_UR50D")
done

experiment_name="esm2_3B"
num_simulations=10
num_iterations=10
...
function run_grid_search() {
    ...
    echo "Running ${dataset_name} dataset:" > out/${dataset_name}-${embedding_type_pt}-esm2_3B_10_rounds.out
    python3 -u grid_search.py \
        ...
        >> out/${dataset_name}-${embedding_type_pt}-esm2_3B_10_rounds.out
    echo "Done running ${dataset_name} dataset:" >> out/${dataset_name}-${embedding_type_pt}-esm2_3B_10_rounds.out
}
