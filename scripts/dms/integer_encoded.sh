...
#SBATCH --time=1:00:00 
#SBATCH --job-name=integer_encoded_optimal
#SBATCH -n 12
#SBATCH -N 1   
#SBATCH --cpus-per-task=5  
#SBATCH --mem=20gb  
#SBATCH --output out/integer_encoded_optimal-%j.out 
...
run_dms_main() {
    dataset_name=$1
    output_file="out/${dataset_name}-integer_encoded_optimal.out"

    echo "Running ${dataset_name} dataset:" > ${output_file}
    python3 -u dms_main.py \
        --dataset_name ${dataset_name} \
        --experiment_name "integer_encoded_optimal" \
        --model_name "integer_encoded" \
