...
#SBATCH --time=1:00:00 
#SBATCH --job-name=ankh_large_optimal
#SBATCH -n 12
#SBATCH -N 1   
#SBATCH --cpus-per-task=5  
#SBATCH --mem=20gb  
#SBATCH --output out/ankh_large_optimal-%j.out 
...
run_dms_main() {
    dataset_name=$1
    output_file="out/${dataset_name}-ankh_large_optimal.out"

    echo "Running ${dataset_name} dataset:" > ${output_file}
    python3 -u dms_main.py \
        --dataset_name ${dataset_name} \
        --experiment_name "ankh_large_optimal" \
        --model_name "ankh_large" \
        --embeddings_path "/orcd/archive/abugoot/001/Projects/Matteo/Github/EvolvePro/output/plm/ankh" \
