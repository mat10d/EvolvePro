...
for model_name in "${model_names[@]}"; do
  ...
  for study in "${study_names[@]}"; do
    command="python3 directed_evolution/extract/esm/extract.py ${model_name} ${fasta_path}${study}.fasta ${results_path}${study}/${model_name_shortened} --toks_per_batch ${toks_per_batch} --include mean"
