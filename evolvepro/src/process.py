...
def process_dataset(
    ...
    generate_mutation_fasta(
        filtered_df, wt_sequence, dataset_name, output_dir, AA_shift
    )
    ...
    for i, (cutoff, label) in enumerate(zip(all_cutoffs, cutoff_labels)):
        ...
        if cutoff_rule == "greater_than":
            filtered_df[column_name] = (filtered_df[activity_column] > cutoff).astype(
                int
            )
            ...
            if cutoff_function is None:
                raise ValueError(
                    "Custom function must be provided for 'custom' cutoff rule."
                )
            filtered_df[column_name] = cutoff_function(
                filtered_df, activity_column, cutoff
            ).astype(int)
        else:
            raise ValueError(
                "Invalid cutoff rule. Please specify 'greater_than', 'less_than', or 'custom'."
            )

        number = filtered_df[column_name].sum()
        fraction = (
            (filtered_df[column_name].sum() / total_values) if total_values > 0 else 0
        )
...
def markin_custom_cutoff(markin_df, activity_column, cutoff):
    ...
    activity_binary = (markin_df[activity_column] > cutoff) & (
        markin_df["kcatOverKM_cMUP_p-value"] < 0.01
    )
...
def preprocess_cas12f(input_cas12f, wt_fasta_output, preprocessed_output_file):
    ...
    # Process non-WT rows
    df = df[df["mean"] != 1].rename(
        columns={"variant": "variant_raw", "mean": "avg_activity"}
    )
...
def generate_n_mutant_combinations(wt_fasta, mutant_file, n, output_file, threshold=1):
    ...
    mutants[["position", "mutant_aa"]] = mutants["Variant"].str.extract(
        "(\d+)([A-Z]+)", expand=True
    )
    mutants["wt_aa"] = mutants.apply(
        lambda row: wt_sequence[int(row["position"]) - 1], axis=1
    )
    ...
    for combination in mutant_combinations:
        ...
        if valid_combination:
            record = SeqRecord(
                Seq(mutant_sequence), id=variant.rstrip("_"), description=""
            )
            records.append(record)
