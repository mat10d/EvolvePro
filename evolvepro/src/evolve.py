        ...
        while j <= num_iterations:
            ...
            if j == 0:
                ...
                if learning_strategy == "dist":
                    iteration_new_ids = (
                        df_test_new.sort_values(by="dist_metric", ascending=False)
                        .head(num_mutants_per_round)
                        .variant
                    )
                elif learning_strategy == "random":
                    iteration_new_ids = random.sample(
                        list(df_test_new.variant), num_mutants_per_round
                    )
                elif learning_strategy == "topn2bottomn2":
                    top_half = (
                        df_test_new.sort_values(by="y_pred", ascending=False)
                        .head(int(num_mutants_per_round / 2))
                        .variant
                    )
                    bottom_half = (
                        df_test_new.sort_values(by="y_pred", ascending=False)
                        .tail(int(num_mutants_per_round / 2))
                        .variant
                    )
                ...
                iteration_new = pd.DataFrame(
                    {"variant": iteration_new_ids, "iteration": j}
                )
                iteration_new = pd.concat(
                    [iteration_new, iteration_old], ignore_index=True
                )
...
def grid_search(
    ...
    if pca_components is not None:
        ...
        embeddings_list = {"embeddings": embeddings, **embeddings_pca}
    else:
                ...
                for mutants_per_round in num_mutants_per_round:
                    ...
                    for embedding_type in embedding_types:
                        ...
                        for regression_type in regression_types:
                            output_results[strategy][var][iterations][
                                mutants_per_round
                            ][embedding_type][regression_type] = {}
                            for first_round_strategy in first_round_strategies:
                                output_results[strategy][var][iterations][
                                    mutants_per_round
                                ][embedding_type][regression_type][
    ...
    if embeddings_type_pt is None:
        df_results.to_csv(
            f"{output_dir}/{dataset_name}_{model_name}_{experiment_name}.csv",
            index=False,
        )
    else:
        df_results.to_csv(
            f"{output_dir}/{dataset_name}_{model_name}_{experiment_name}_{embeddings_type_pt}.csv",
            index=False,
        )
...
def evolve_experimental(
    ...
    for round_file_name in round_file_names:
        experimental_data = load_experimental_data(
            round_base_path, round_file_name, wt_fasta_path
        )
        all_experimental_data.append(experimental_data)
        print(
            f"Loaded experimental data for {round_file_name}: {experimental_data.shape}"
        )

    # Create iteration dataframes
    iteration, labels = create_iteration_dataframes(
        all_experimental_data, embeddings.index
    )
...
def evolve_experimental_multi(
    ...
    for i, file_name in enumerate(embeddings_file_names):
        embedding = load_experimental_embeddings(
            embeddings_base_path, file_name, rename_WT
        )
        if i > 0:  # If not the first file
            embedding = embedding[
                ~embedding.index.isin(["WT", "WT Wild-type sequence"])
            ]
    ...
    print(f"\nTop {number_of_variants} variants predicted by the model:")
    print(
        df_test.sort_values(by=["y_pred"], ascending=False).head(number_of_variants)[
            ["variant", "y_pred"]
        ]
    )
